...
do not have too much repetitive information, numerical values using the original numbers.
        ...
        summarizes.append(gpt_say)
        file_write_buffer.extend([i_say, gpt_say])
        ############################## <第 4 步，设置一个token上限> ##################################
        _, final_results = input_clipping("", final_results, max_token_limit=3200)
        yield from update_ui(chatbot=chatbot, history=gpt_say)  # 注意这里的历史记录被替代了

    i_say, summarizes = input_clipping("请从方法，原理，实验设置，实验精度等方面比较以上论文并用表格显示", summarizes, max_token_limit=2000)
    gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
        inputs=i_say, inputs_show_user='开始最终比较',
        llm_kwargs=llm_kwargs, chatbot=chatbot, history=summarizes,
        sys_prompt=f"Display a table"
    )
    ...
    yield from update_ui(chatbot=chatbot, history=summarizes)  # 刷新界面


@CatchException
def 批量文献综述(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    import glob, os
